 ...
 * Uses a typed global declaration to avoid @ts-ignore.
 */
const GLOBAL_CRON_KEY = Symbol.for('automation.scheduler.initialized');

// Extend globalThis with a typed property for our singleton flag
declare global {
  // eslint-disable-next-line no-var
  var __automationSchedulerInitialized: true | undefined;
}
...
export async function initAutomationScheduler() {
  ...
  if (globalThis.__automationSchedulerInitialized) {
    return;
  }
  globalThis.__automationSchedulerInitialized = true;
